...
- 검수 항목은 **FAT / SAT 별로 다른 체크리스트**를 사용합니다. `app.js`의
  `FAT_TEMPLATE`(공장검수, 36항목)과 `SAT_TEMPLATE`(현장검수, 40항목)에
  카테고리별로 정의되어 있고, 카테고리 표시 정보는 `CATEGORY_META`에
  있습니다. FAT는 제작처 출하 전 "장비 자체"를 확인하고, SAT는 설치 후
  실제 라인·유틸리티·상위시스템까지 물려 놓고 생산·품질·설비보전·
  안전환경·정보시스템 등 여러 부서가 함께 확인하므로 항목이 더 세분화되어
  있으며 각 항목에 확인 담당 부서가 배지로 표시됩니다. 현장에 맞게
  이름/항목/부서를 자유롭게 수정하세요.
- **최종 합격/불합격 판정**은 `app.js`의 `NG_LIMITS`에 정의된 "항목군별
  허용 NG(불합격) 개수"를 기준으로 자동 계산됩니다. 어느 항목군이든 허용치를
  초과하는 NG가 나오면 최종 불합격, 허용치 이내면 (NG가 있어도) 조건부 합격,
  NG가 없으면 합격입니다. `0`은 무관용(1건이라도 NG면 불합격)이며 기본값은
  안전·유틸리티 0, 나머지 1(문서 2)입니다. 기준값은 새 검수를 만들 때
  기록에 함께 저장되므로 나중에 기준을 바꿔도 과거 검수 판정은 그대로
  유지됩니다. 체크리스트·요약·인쇄 보고서에 항목군별 NG/허용치와 최종
  판정이 표시됩니다.
